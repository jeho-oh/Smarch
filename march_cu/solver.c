...
#define CUBE_INC	1.01
#define CUBE_DEC	0.86

#define NEWSTYLE
#define DECISIONSONLY

#define TH_DEPTH	15
#define FACTOR_MAX	4000.0
//#define TH_FACTOR	1.0
...
int initSolver()
{
	...
	nr_assigned = 0;
	conflicts   = 0;
//	refute_compensation = 0;
	factor_th = FACTOR_MAX;
        free_th   = 0.0;
        sum_th    = 0;
	nr_cubes  = 0;
...
#ifdef DISTRIBUTION // to distribution.c
...
//        if (freevars < free_th)  // new default
      if ((cut_depth && (depth == cut_depth)) || (!cut_depth && (freevars < free_th)))
      {
	nodeCount--;
	nr_cubes++;
...
//        free_th *= (1.0 - pow(fraction, sqrt(depth)));
	return UNSAT; }
 }
#endif
#ifdef DISTRIBUTION
...
#endif
#ifdef CUBE
	if (progress_th == depth && progress_th < 10) {
		progress_th++; }
#endif
	NODE_START();
#ifdef BLOCK_PRESELECT
...
#endif
//	printf("FIXING %i at depth %i (%i)\n", nrval, depth, nr_assigned );
